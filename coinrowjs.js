	...
	function calcGraphLocations(){
			graph_min_y = GRAPH_PERCENT_FROM_EDGES*canvas.height;
			graph_max_y = canvas.height - GRAPH_PERCENT_FROM_EDGES*canvas.height;
			
			graph_min_x = GRAPH_PERCENT_FROM_EDGES*canvas.width;
			graph_max_x = canvas.width - GRAPH_PERCENT_FROM_EDGES*canvas.width;
			
			graph_height = getGraphPix(canvas.height);
			graph_width = getGraphPix(canvas.width);
			
			graph = {min_y:graph_min_y, max_y:graph_max_y,
						min_x:graph_min_x, max_x:graph_max_x,
	...
	function drawOutline(){
			...
			ctx.strokeRect(graph.min_x, graph.min_y, graph.width, graph.height);
	}
